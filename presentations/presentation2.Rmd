...
messi_heatmap <- messi_heatmap |>
  filter(type.name=="Dribbled Past" | type.name=="Nutmeg") |>
  group_by(season_year) |>
  mutate(total_dribbles = n()) |>
  group_by(season_year, xbin, ybin) |>
  summarise(total_dribbles = max(total_dribbles),
    bin_dribbles = n(),
    bin_pct = bin_dribbles/total_dribbles,
    location.x = median(location.x),
    location.y = median(location.y)) |>
  group_by(xbin, ybin) |>
  mutate(messi_avg = mean(bin_pct)) |>
  group_by(season_year, xbin, ybin) |>
  mutate(diff_vs_ave = bin_pct - messi_avg)

library(grid)

dribblecolor <- c("#dc2429", "#dc2329", "#df272d", "#df3238", "#e14348", "#e44d51",
...
ggplot(data= messi_heatmap, aes(x = location.x, y = location.y, fill = diff_vs_ave, group =diff_vs_ave)) +
...
family="Source Sans Pro", colour = "black", hjust = 0.5),
 ...
 scale_y_reverse() + #5
 scale_fill_gradientn(colours = dribblecolor, trans = "reverse", labels =
